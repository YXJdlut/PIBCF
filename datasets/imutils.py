...
def random_crop2(image, image_vis, image_mask, label, crop_size, mean_rgb=[0, 0, 0], ignore_index=255):
    ...
    # 获取图像的原始尺寸
    h_vis, w_vis = image_vis.shape[:2]
    
    # 确保输入的图像都是三通道的
    if len(image.shape) == 2:  # 如果输入的图像是灰度图（单通道）
        image = np.expand_dims(image, axis=-1)  # 增加一个通道维度
        image = np.repeat(image, 3, axis=-1)  # 转换为三通道图像
    
    if len(image_vis.shape) == 2:  # 如果 `image_vis` 是灰度图
        image_vis = np.expand_dims(image_vis, axis=-1)  # 增加一个通道维度
        image_vis = np.repeat(image_vis, 3, axis=-1)  # 转换为三通道图像

    if len(image_mask.shape) == 2:  # 如果 `image_mask` 是灰度图
        image_mask = np.expand_dims(image_mask, axis=-1)  # 增加一个通道维度
        image_mask = np.repeat(image_mask, 3, axis=-1)  # 转换为三通道图像
    H_pad = int(np.random.randint(H - h + 1))
    W_pad = int(np.random.randint(W - w + 1))
