...
class GeSeNet(nn.Module):
    ...
    def forward(self, image_vis, image_ir):
        # 🔹 确保是 [B,1,H,W]
        if image_vis.dim() == 3:
            image_vis = image_vis.unsqueeze(1)
        if image_ir.dim() == 3:
            image_ir = image_ir.unsqueeze(1)
        ...
        if isinstance(x_vis_p1, tuple):   # 有些模块返回 (out, mask)
            x_vis_p1 = x_vis_p1[0]
        x_vis_p2 = self.vis_rmm(x_vis_p1)
        if isinstance(x_vis_p2, tuple):
            x_vis_p2 = x_vis_p2[0]

        # --- INF branch ---
        x_inf_p = self.inf_conv(x_inf_origin)
        x_inf_p1 = self.inf_eem(x_inf_p)
        if isinstance(x_inf_p1, tuple):
            x_inf_p1 = x_inf_p1[0]
        x_inf_p2 = self.inf_rmm(x_inf_p1)
        if isinstance(x_inf_p2, tuple):
            x_inf_p2 = x_inf_p2[0]

        # --- 保证维度 ---
        if x_vis_p2.dim() == 3:
            x_vis_p2 = x_vis_p2.unsqueeze(-1)
        if x_inf_p2.dim() == 3:
            x_inf_p2 = x_inf_p2.unsqueeze(-1)

        x = torch.cat((x_vis_p2, x_inf_p2), dim=1)

        # 🔑 spatial 对齐 (H,W)
        if x.shape[2:] != x_inf_p1.shape[2:]:
            x = F.interpolate(x, size=x_inf_p1.shape[2:], mode="bilinear", align_corners=False)
