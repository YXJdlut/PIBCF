...
class RMC(nn.Module):
    ...
    def _prepare(self):
        ...
        # kernel split
        kernel_d2d, kernel_d2s, kernel_s = [], [], []
    ...
    def _generate_indices(self, spa_mask):
        ...
        indices = torch.arange(float(mask_indices.size(0))).view(1, -1).to(spa_mask.device) + 1
        spa_mask_clone = spa_mask.clone()
        spa_mask_clone[0, 0, self.h_idx_1x1, self.w_idx_1x1] = indices

        self.idx_s2s = F.pad(spa_mask_clone, [1, 1, 1, 1])[0, :, self.h_idx_3x3, self.w_idx_3x3].view(9, -1).long()
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            spa_mask = x[1]
            self._prepare()              # 🔑 必须先准备好 d_in_num 等
            self._generate_indices(spa_mask)

            fea_d = x[0]
            fea_s = None
            fea_dense, fea_sparse = [], []
            for i in range(self.n_layers):
