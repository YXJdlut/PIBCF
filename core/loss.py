...
class Fusionloss(nn.Module):
    def __init__(self, clip_weight=1.0):
        super(Fusionloss, self).__init__()
        self.sobelconv = Sobelxy()  # Sobel 梯度计算
        self.de1L_clip = L_clip()  # 实例化 L_clip
        self.clip_weight = clip_weight  # CLIP 损失的权重      
    ...
    def forward(self,image_ir,image_vis,generate_img,mask):
        ...
        # x_in_max=(image_y+image_ir)/2  #####可以修改
        ...
        loss_clip = self.de1L_clip(generate_img)  # 计算 CLIP 损失
        loss_clip = self.clip_weight * loss_clip  # 加权 CLIP 损失
        loss_total=(loss_in + 2*loss_grad)*5 + loss_clip
        return loss_total
...
#         loss_clip = self.de1L_clip(generate_img)  # 计算 CLIP 损失
#         loss_clip = self.clip_weight * loss_clip  # 加权 CLIP 损失
#         return loss_in + loss_grad+loss_clip
import torch
import torch.nn as nn
import torch.nn.functional as F
from pytorch_ssim import ssim
...
class FusionlossHalf(nn.Module):
    def __init__(self, clip_weight=1.0, ssim_weight=2.0):
        super(FusionlossHalf, self).__init__()
        self.sobelconv = Sobelxy()
        self.de1L_clip = L_clip()
        self.clip_weight = clip_weight
        self.ssim_weight = ssim_weight

    # 转成单通道（如果是 3 通道伪灰度图，取第一个通道即可）
    def to_gray(self, x):
        if x.size(1) == 3:
            return x[:, 0:1, :, :]
        return x
    ...
    def forward(self, image_ir, image_vis, generate_img, mask):
        # ----------- MSE -----------
        gen_gray  = self.to_gray(generate_img)
        mask_gray = self.to_gray(mask)
        ir_gray   = self.to_gray(image_ir)
        vis_gray  = self.to_gray(image_vis)

        loss_mse_mask = F.mse_loss(gen_gray, mask_gray)
        loss_mse_input = 0.5 * (F.mse_loss(gen_gray, ir_gray) +
                                F.mse_loss(gen_gray, vis_gray))
        loss_mse = 0.5 * (loss_mse_mask + loss_mse_input)

        # ----------- SSIM -----------
        gen_norm  = gen_gray * 0.5 + 0.5
        mask_norm = mask_gray * 0.5 + 0.5
        ir_norm   = ir_gray * 0.5 + 0.5
        vis_norm  = vis_gray * 0.5 + 0.5

        loss_ssim_mask = 1 - ssim(gen_norm, mask_norm)
        loss_ssim_input = 0.5 * (
            (1 - ssim(gen_norm, ir_norm)) +
            (1 - ssim(gen_norm, vis_norm))
        )
        loss_ssim = 0.5 * (loss_ssim_mask + loss_ssim_input)

        # ----------- Gradient -----------
        gen_grad  = self.sobelconv(gen_gray)
        mask_grad = self.sobelconv(mask_gray)
        ir_grad   = self.sobelconv(ir_gray)
        vis_grad  = self.sobelconv(vis_gray)

        loss_grad_mask = F.mse_loss(gen_grad, mask_grad)
        loss_grad_input = 0.5 * (F.mse_loss(gen_grad, ir_grad) +
                                 F.mse_loss(gen_grad, vis_grad))
        loss_grad = 0.5 * (loss_grad_mask + loss_grad_input)

        # ----------- CLIP -----------
        # CLIP 部分还是对生成图的原始三通道做
        loss_clip = self.clip_weight * self.de1L_clip(generate_img)

        # ----------- 总损失 -----------
        total_loss = loss_mse + self.ssim_weight * loss_ssim + loss_grad + 0.1 * loss_clip
...
class Fusionloss3(nn.Module):
    def __init__(self, clip_weight=1.0, ssim_weight=2.0):
        ...
        self.de1L_clip = L_clip()  # 实例化 L_clip
        self.clip_weight = clip_weight  # CLIP 损失的权重
        self.ssim_weight = ssim_weight  # SSIM 损失的权重

    def forward(self, image_ir, image_vis, generate_img, mask):
        loss_mse = F.mse_loss(mask, generate_img)

        # 计算 SSIM Loss
        generate_img_normalized = generate_img * 0.5 + 0.5  # 归一化到 [0, 1]
        mask_normalized = mask * 0.5 + 0.5  # 归一化到 [0, 1]
        loss_ssim = 1 - ssim(generate_img_normalized, mask_normalized)  # SSIM 损失

        # 计算梯度损失
        generate_img_grad = self.sobelconv(generate_img)
        mask_grad = self.sobelconv(mask)
        loss_grad = F.mse_loss(mask_grad, generate_img_grad)  # 使用 MSE Loss 替换 L1 Loss
        loss_clip = self.de1L_clip(generate_img)
        loss_clip = self.clip_weight * loss_clip

        total_loss = loss_mse + self.ssim_weight * loss_ssim + loss_grad + 0.1 * loss_clip
        # total_loss = loss_mse + self.ssim_weight * loss_ssim  + 0.1 * loss_clip
        # total_loss = loss_mse + self.ssim_weight * loss_ssim  + 0.1 * loss_clip
...
class Fusionloss4(nn.Module):
    ...
    def forward(self, image_ir, image_vis, generate_img, mask=None):
        # ---- MSE Loss（和输入图像对比）----
        loss_mse_ir = F.mse_loss(generate_img, image_ir)
        loss_mse_vis = F.mse_loss(generate_img, image_vis)
        loss_mse = 0.5 * (loss_mse_ir + loss_mse_vis)

        # ---- SSIM Loss（和输入图像对比）----
        gen_norm = generate_img * 0.5 + 0.5
        ir_norm  = image_ir * 0.5 + 0.5
        vis_norm = image_vis * 0.5 + 0.5
        loss_ssim_ir = 1 - ssim(gen_norm, ir_norm)
        loss_ssim_vis = 1 - ssim(gen_norm, vis_norm)
        loss_ssim = 0.5 * (loss_ssim_ir + loss_ssim_vis)

        # ---- 梯度 Loss（和输入图像对比）----
        gen_grad = self.sobelconv(generate_img)
        ir_grad  = self.sobelconv(image_ir)
        vis_grad = self.sobelconv(image_vis)
        loss_grad_ir = F.mse_loss(gen_grad, ir_grad)
        loss_grad_vis = F.mse_loss(gen_grad, vis_grad)
        loss_grad = 0.5 * (loss_grad_ir + loss_grad_vis)

        # ---- CLIP Loss（保持原来，只对生成图）----
        loss_clip = self.de1L_clip(generate_img)
        loss_clip = self.clip_weight * loss_clip
